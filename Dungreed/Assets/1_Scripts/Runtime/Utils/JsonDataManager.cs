...
public class JsonDataManager : Singleton<JsonDataManager>
{
    ...
    {
        File.WriteAllText(Path.Combine(Application.streamingAssetsPath, path, fileName + ExtensionName), JsonUtility.ToJson(obj, true));
    }

    public bool LoadFromJson<T>(string path, string fileName, out T val)
    {
        string json = File.ReadAllText(Path.Combine(Application.streamingAssetsPath, path, fileName + ExtensionName), Encoding.UTF8);
